...
Lite: it walks the whole admin experience (fresh DB → seed → login →
dashboard → create a user → see it listed + audit-logged → logout) and
...
Core Lite ships with public registration **disabled**. To enable it for a
product, set:
