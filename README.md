...
Penova Core Lite یک هستهٔ رایگان برای ساخت محصولات Laravel است؛ یک Laravel Product Factory Starter که چیزهای تکراری اغلب پروژه‌ها (احراز هویت، کاربران، نقش‌ها، تنظیمات، نوتیفیکیشن‌ها، لاگ فعالیت، UI و DataTable) را یک‌بار برای همیشه حل می‌کند و آماده است تا ماژول‌های محصولی مانند Store، SMS و Payment روی آن نصب شوند.

Penova Core Lite is a free foundation for building Laravel products. It ships a production-ready admin core (auth, users, roles, settings, notifications, activity log, UI components and a reusable DataTable) so you can focus on your product modules.

این مخزن فقط Core Lite است؛ هیچ ماژول بیزنسی (فروشگاه، CRM، Booking و غیره) در آن وجود ندارد.  
...
## چه چیزهایی داخل Core Lite نیست؟

برای این‌که Core Lite خالص و product-agnostic بماند، موارد زیر عمداً در این مخزن نیستند:
...
این‌ها به صورت ماژول‌های جداگانه روی Core Lite نصب می‌شوند.
...
```bash
git clone https://github.com/your-vendor/penova-core-lite.git
cd penova-core-lite
...
- صفحه اصلی (`/`) صفحه Welcome Penova Core Lite را نشان می‌دهد.  
...
اگر چیزی تنظیم نکنید، Core Lite از مقادیر پیش‌فرض در config/penova.php استفاده می‌کند.

## افزودن ماژول‌ها (Store، SMS، Payment)

Core Lite از ابتدا برای نصب ماژول‌های بیزنسی آماده است.
...
- app/Modules  
  - در Core Lite خالی است؛ ماژول‌ها را جداگانه اضافه می‌کنید.
...
- resources/js/Core  
  ...
  - Welcome.vue: صفحهٔ خوش‌آمد Core Lite در روت `/`
...
اگر به یک هستهٔ مشترک برای چند محصول نیاز دارید، Penova Core Lite برای همین طراحی شده است.
...
- ماژول‌های CRM / Booking عمداً در Core Lite نیستند و اگر اضافه شوند، به‌صورت ماژول مستقل خواهند بود، نه بخشی از Core.
